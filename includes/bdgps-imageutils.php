...
require_once(dirname(__FILE__) . "/bdgps-access.php");

define("BDGPS_RESIZE_IMAGE_TEMPFILE", BDGPS_CACHE_DIR . "/" . BDGPS_genUniqueCode() . ".img");
define("BDGPS_RESIZE_IMAGE_ESCAPESIGN", "---");

class BDGPS_ImageUtils {
	...
	function generate_cache_filename($src, $width, $height, $quality) {
		...
		$escaped = str_replace(BDGPS_RESIZE_IMAGE_ESCAPESIGN, BDGPS_RESIZE_IMAGE_ESCAPESIGN . BDGPS_RESIZE_IMAGE_ESCAPESIGN, $normalized);
		$masked = str_replace("/", BDGPS_RESIZE_IMAGE_ESCAPESIGN, $escaped);
		$widthed = $masked . BDGPS_RESIZE_IMAGE_ESCAPESIGN . $width;
		$heighted = $widthed . BDGPS_RESIZE_IMAGE_ESCAPESIGN . $height;
		$qualitied = $heighted . BDGPS_RESIZE_IMAGE_ESCAPESIGN . $quality;
		return $qualitied;
	}
	...
	function generate_timestamped_cache_filename($src, $width, $height, $quality, $timestamp) {
		$timestamped = BDGPS_ImageUtils::generate_cache_filename($src, $width, $height, $quality) . BDGPS_RESIZE_IMAGE_ESCAPESIGN . $timestamp;
		return $timestamped;
	}
	...
	function get_type($file) {
		list(, $type) = split("/", BDGPS_ImageUtils::get_mime_type($file));
		return $type;
	}
	...
	function get_create_function_name($file) {
		return "imagecreatefrom" . BDGPS_ImageUtils::get_type($file);
	}
	...
	function get_send_function_name($file) {
		return "image" . BDGPS_ImageUtils::get_type($file);
	}
	...
	function generate_resized_image($file, $width, $height) {
		...
		$create_function_name = BDGPS_ImageUtils::get_create_function_name($file);
	...
	function get_cached_file($src, $width, $height, $quality) {
		$testfile = BDGPS_ImageUtils::generate_cache_filename($src, $width, $height, $quality);

		$cached_file = null;
		$dh = opendir(BDGPS_CACHE_DIR);
		...
		closedir($dh);
		return ($cached_file == null) ? null : BDGPS_CACHE_DIR . "/" . $cached_file;
	}
	...
	function remove_cached_file($src, $width, $height, $quality) {
		$cached_file = BDGPS_ImageUtils::get_cached_file($src, $width, $height, $quality);
	...
	function get_mtime_from_cached_file($file) {
		$parts = explode(BDGPS_RESIZE_IMAGE_ESCAPESIGN, $file);
	...
	function cached_file_is_needed($src, $width, $height, $quality, $timestamp) {
		...
		$cached_file_timestamp = BDGPS_ImageUtils::get_mtime_from_cached_file($cached_file);
